...
<%@ page import="training.pack.Tpojo.*, training.pack.Tdao.HomeDao" %>
<%@ page import="java.util.*" %>

<%
HomeDao h1 = new HomeDao();
List<ProductPojo> vList = h1.fetchAllProduct();
String loggedInUser = (String) session.getAttribute("user");

// Razorpay Client Setup
RazorpayClient razorpay = new RazorpayClient("rzp_test_fI2kIVIDH3X305", "5z4BelDDTqXvaEMZKNnHBrNn");

Map<String, String> orderMap = new HashMap<>(); // Store order IDs for products
%>
...
<body>
    ...
    <% if (loggedInUser != null) { %>  
        ...
        <ul>
        <% for (ProductPojo vid : vList) { 
        	 String videoId = String.valueOf(vid.getId());
        	 System.out.println( videoId );
        	 if(vid.getId().equals(videoId)) {
           
            double amountInInr = vid.getProductPrice(); // If getProductPrice() returns a double
            int amountInPaise = (int) (amountInInr * 100); // Convert to paise
            String productName = vid.getProductName();
            String orderId = "";
            ...
            try {
                ...
                orderRequest.put("receipt", "txn_" + videoId);
                orderRequest.put("payment_capture", 1);

                Order order = razorpay.orders.create(orderRequest);
                orderId = order.get("id"); // Store the order ID
                orderMap.put(videoId, orderId); // Store order ID for this video
    ...
    <li>
        <p><strong><%= productName %></strong> - &#8377;<%= amountInInr %></p>
    ...
    <% } else { %>
        <p style="color: red; text-align: center;">User not logged in. Please log in to continue.</p>
    <% } %>
